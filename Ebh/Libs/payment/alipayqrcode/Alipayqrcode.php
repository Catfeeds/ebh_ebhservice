...
class Alipayqrcode implements Payment{
    ...
    public function getPaymentCode($order,$parameters = array()){
        ...
        $alipayParams = array(
            ...
            //二维码参数
            "qr_pay_mode"=>4,
            "qrcode_width"=>isset($parameters['width']) && $parameters['width']>0?$parameters['width']:93
        );
    ...
    public function notify($request = array()){
        ...
        $order = Transaction::notifyOrder($_POST['out_trade_no'],$_POST['trade_no'],$param);
        if($order['status'] == 1) {//订单已处理，则不重复处理
            //写缓存用于前端验证刷新
            $redis = Ebh()->cache->getRedis();

            $ordernumKey = md5($_POST['out_trade_no']);
            $redis->set($ordernumKey,1);
            log_message("支付宝支付成功:".$_POST['out_trade_no']);
        }
        echo "success";
        exit;
    }
